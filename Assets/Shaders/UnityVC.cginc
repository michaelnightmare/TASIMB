...
struct VertexOutputForwardBase_VC
{
	...
	float4 reflUVW				: TEXCOORD9;
	#else
	float4 reflUVW				: TEXCOORD8;
	#endif
#endif
...
VertexOutputForwardBase_VC vertForwardBase_VC(VertexInput_VC v)
{
	...
	float4 viewDirForParallax = mul(rotation, ObjSpaceViewDir(v.vertex));
...
struct VertexOutputForwardAdd_VC
{
	...
	UNITY_FOG_COORDS(7)
		...
		float4 viewDirForParallax			: TEXCOORD8;
#endif
	fixed4 color : COLOR;
...
#if UNITY_OPTIMIZE_TEXCUBELOD
#if UNITY_SPECCUBE_BOX_PROJECTION
	float4 reflUVW				: TEXCOORD7;
#else
	float4 reflUVW				: TEXCOORD6;
#endif
#endif
...
)
{
	...
	float4 color = UNITY_BRDF_PBS(s.diffColor, s.specColor, s.oneMinusReflectivity, s.smoothness, s.normalWorld, -s.eyeVec, gi.light, gi.indirect).rgb;
#if _VERTEXCOLOR
	color *= i.color * _IntensityVC;
#endif
#if _VERTEXCOLOR_LERP
	color *= lerp(float4(1, 1, 1), i.color.rgb, _IntensityVC);
#endif
